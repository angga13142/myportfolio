...
function Particles({
  ...
  // Reduce particles di mobile, disable di low-end
  const actualQuantity = isMobile ? Math.min(quantity, 15) : quantity;

  useEffect(() => {
    // Delay particle initialization for faster initial load
  ...
  // Pause animasi saat tab tidak aktif (save CPU)
  useEffect(() => {
  ...
  // Disable particles di low-end devices
  if (isLowEnd) {
    return null;
  }
